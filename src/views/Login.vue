<template>
  <div class="login-wrap">
    <el-form label-position="left" :model="ruleForm" :rules="rules" ref="ruleFormRef" label-width="0px"
      ...
      <el-form-item prop="captcha">
        <el-input type="text" size="large" v-model.trim="ruleForm.captcha" auto-complete="off"
                  @keyup.enter="submitForm" placeholder="验证码">
          <template #prepend>
            <el-icon>
              <circle-check/>
            </el-icon>
      ...
      <el-form-item style="width:100%;margin-top: 10px">
        <el-button type="primary" size="large" :loading="loadingLg" style="width:100%;"
                   @click="submitForm">登录
        </el-button>
      </el-form-item>
    </el-form>
...
import {delCookie, getCookie, setCookie} from '@/utils/common'
import {apiCaptcha, apiLogin} from '@/utils/api'
...
function getUserPassword() {
  if (getCookie('username') !== '' && getCookie('password') !== '') {
    ...
    ruleForm.password = getCookie('password')
...
function getCaptcha() {
  apiCaptcha().then((ret) => {
    ...
    ruleForm.captchaKey = ret.data.data.key
    image_base.value = ret.data.data.image_base
...
function submitForm() {
  ruleFormRef.value.validate().then(() => {
    loadingLg.value = true
    // console.log('aaa',ruleForm)
    apiLogin(ruleForm).then(async res => {
      loadingLg.value = false
      // console.log('apiLogin',res)
      // console.log('rememberpassword',rememberpassword)
      if (res.data.code === 2000) {
        if (rememberpassword.value) {
          //保存帐号到cookie，有效期7天
          await setCookie('username', ruleForm.username, 7)
          //保存密码到cookie，有效期7天
          await setCookie('password', ruleForm.password, 7)
        } else {
        ...
        console.log("登录成功。")
        store.dispatch('login', {username: ruleForm.username, name: res.data.data.name, avatar: res.data.data.avatar})
        router.push('/')
    ...
    ElMessage.error('请输入完整的用户名/密码/验证码！')
    return false
  })
}
